...
int main() {
    ...
    char* base_filepath =
            "/mnt/d/VectorDB/sift/sift/sift_base.fvecs";
    char* query_filepath =
            "/mnt/d/VectorDB/sift/sift/sift_query.fvecs";
    size_t dd; // dimension
    ...
    {
        srand((int)time(0));
        std::vector<float> trainvecs(nb / train_ration * d);
        for (int i = 0; i < nb / train_ration; i++) {
            int rng = (rand() % (nb + 1));
        ...
        int* gt_int = ivecs_read(
                "/mnt/d/VectorDB/sift/sift/sift_groundtruth.ivecs",
                &kk,
                &nq2);
        ...
        faiss::IndexIVFPQFastScan index(
                &coarse_quantizer, 128, nlist, 64, 4, faiss::METRIC_L2);
        ...
        newindex.train(nb / train_ration, trainvecs.data());
