...
int main() {
    ...
    int ratio = 50;
    {
        ...
        printf("[%.3f s] IndexIVFFlat start to train\n", elapsed() - t0);
        index.train(nb / 100, trainvecs.data());
        printf("[%.3f s] IndexIVFFlat train finished\n", elapsed() - t0);
        // assert(index.is_trained);
        printf("[%.3f s] IndexIVFFlat start to add\n", elapsed() - t0);
        index.add(nb, xb);
        printf("[%.3f s] IndexIVFFlat add finished\n", elapsed() - t0);
        ...
        { // search xq
            ...
            for (int i : arr) {
                ...
                index.nprobe = i;
                // params.nprobe=i;
                printf("[%.3f s] IndexIVFFlat start to search\n",
                       elapsed() - t0);
                ...
                //  }
                printf("[%.3f s] IndexIVFFlat search finished\n",
                       elapsed() - t0);
