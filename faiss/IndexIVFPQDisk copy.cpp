...
// Struct to hold the mmap related data
struct MmapFile {
    int fd;           // File descriptor
    char* data;       // Pointer to the mapped data
    size_t file_size; // Size of the mapped file
};

// Function to initialize mmap
MmapFile initialize_mmap(const std::string& file_path) {
    MmapFile mmap_file;

    // Open the file
    mmap_file.fd = open(file_path.c_str(), O_RDONLY);
    if (mmap_file.fd == -1) {
    ...
    // Get the file size
    struct stat sb;
    if (fstat(mmap_file.fd, &sb) == -1) {
        close(mmap_file.fd);
        throw std::runtime_error("Failed to get file size");
    }
    mmap_file.file_size = sb.st_size;

    // Map the file into memory
    mmap_file.data = static_cast<char*>(mmap(nullptr, mmap_file.file_size, PROT_READ, MAP_PRIVATE, mmap_file.fd, 0));
...
struct IVFPQDiskScanner : IVFPQDiskScannerT<idx_t, METRIC_TYPE, PQDecoder>,
    ...
    int precompute_mode;
    const IDSelector* sel;
    mutable std::ifstream disk_data;
    float* raw_query  = nullptr;
        MmapFile mmap_file;  // Struct to hold mmap-related data
    ...
    IVFPQDiskScanner(
        ...
        std::string file_path = ivfpq_disk.get_disk_path();
        mmap_file = initialize_mmap(file_path);
    ...
    size_t scan_codes(
        ...
        // TODO: 1. 先扫描计算一个聚类(list)中的全部PQ码,得到每个向量的大概距离
        //       2. 把这些大概距离放进堆里面
        //       3. 从磁盘读取向量
        ...
        if(real_heap != 0){
            ...
            // std::string file_path = this->ivfpq_disk.get_disk_path();
            ...
            if(this->load_strategy == FULLY){
                ...
                // Directly access the data from the memory-mapped file
                const float* cluster_data = reinterpret_cast<const float*>(mmap_file.data + offset);
                auto end = std::chrono::high_resolution_clock::now();
                std::chrono::duration<double> duration = end - start;
                indexIVFPQDisk_stats.full_cluster_io_time += duration.count();
                // float* cluster_data = vec.data();
 
                for(size_t i = 0; i < real_heap; i++ ){
                    /*
                    根据比较的情况，来决定是否进行rerank
                    */
                    if (list_sim[i] < heap_sim[0] * factor) {
                        //获取当前的id,从而得到对应向量的起始位置
                ...
                // disk_data.seekg(offset, std::ios::beg);
                for(size_t i = 0; i < real_heap; i++ ){
                    // Calculate the offset for the current vector
                     size_t vector_offset = offset + list_ids[i] * single_offset;
                    /*
                    根据比较的情况，来决定是否进行rerank
                    */
                    disk_data.seekg(offset, std::ios::beg);
                    if (list_sim[i] < heap_sim[0] * factor_partial) {
                        ...
                        const float* vector_data = reinterpret_cast<const float*>(mmap_file.data + vector_offset);
                        std::memcpy(vec.data(), vector_data, D * sizeof(float));
                        auto end = std::chrono::high_resolution_clock::now();
                        std::chrono::duration<double> duration = end - start;
                        indexIVFPQDisk_stats.partial_cluster_io_time += duration.count();
                        float distance = fvec_L2sqr(raw_query, vec.data(), D);
                        res.add(list_ids[i], distance);
                        stats_rerank++;
                        if (C::cmp(temp_result, distance))
