...
void disk_full_ifs(
    ...
    float threshold = C::cmp(list_sim[0],heap_sim[0])?heap_sim[0]:list_sim[0];
    ...
    compute_precise_dis_simd(query, cluster_data, p_begin, D, distances.data(),vec_num);
    for(size_t i = 0; i < vec_num; i++){
...
void disk_all_mmap(
    ...
    compute_precise_dis_simd(query, vec_data.data(), temp_pos.data(), D, distances.data(),vec_num);
