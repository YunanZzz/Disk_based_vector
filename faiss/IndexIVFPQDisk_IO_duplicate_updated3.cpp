...
void disk_full_fread(
    ...
    // if (read_count != D * len) {
    //     throw std::runtime_error("Failed to read data from disk file");
    // }
    ...
    FILE* disk_data)
{
    ...
    // 使用 fseek 移动到 cluster_begin 对应的偏移量
    ...
    for (size_t i = 0; i < real_heap; i++) {
        /*
        根据比较的情况，来决定是否进行 rerank
        */
        // 在每次循环中使用 fseek 和 fread 读取数据
        if (list_sim[i] < heap_sim[0] * factor_partial) {

            auto time_start = std::chrono::high_resolution_clock::now();  // time begin

            // 定位到对应的 list_ids[i] 偏移
            fseek(disk_data, offset, SEEK_SET);
            fseek(disk_data, list_ids[i]* single_offset, SEEK_CUR);
            // if (fseek(disk_data, list_ids[i] * single_offset, SEEK_CUR) != 0) {
            //     throw std::runtime_error("Failed to seek in disk file");
            // }
...
)
{
    ...
    // 使用 seekg 来定位到 cluster_begin 对应的偏移量
    // disk_data.seekg(offset, std::ios::beg);
    // if (!disk_data) {
    //     throw std::runtime_error("Failed to seek in disk file");
    // }
...
void disk_full_read(
    ...
    // if (bytesRead != static_cast<ssize_t>(D * len * sizeof(float))) {
    //     throw std::runtime_error("Failed to read data from disk file");
    // }
    ...
    for (size_t i = 0; i < real_heap; i++) {
        ...
        */
        if (list_sim[i] < heap_sim[0] * factor_partial) {
            ...
            // if (lseek(disk_fd, offset + list_ids[i] * single_offset, SEEK_SET) == -1) {
            //     throw std::runtime_error("Failed to seek in disk file");
            // }

            // 读取数据
            ssize_t bytesRead = read(disk_fd, vec.data(), D * sizeof(float));
            // if (bytesRead != static_cast<ssize_t>(D * sizeof(float))) {
            //     throw std::runtime_error("Failed to read data from disk file");
            // }
...
void disk_full_mmap(
    ...
    //std::vector<float> vec(D*len);
    //std::memcpy(vec.data(), cluster_data, D * len * sizeof(float));
